...
final class ShieldPageScript {
    ...
    static String documentStart(boolean enabled,
        ...
        return "(function(){'use strict';try{"
                ...
                + "function clickGuard(e){if(!S.config.enabled||!S.config.click)return;try{var u=targetUrl(e.target);if(u&&safeReaderNav(u,e.target))return;if(u&&bad(u,e.target))return deny(e,u);}catch(x){}}"
