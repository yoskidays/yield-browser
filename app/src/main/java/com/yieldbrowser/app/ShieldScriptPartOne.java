...
final class ShieldScriptPartOne {
    ...
    static String value() {
        return "function low(v){return String(v||'').toLowerCase();}"
                ...
                + "function safeDownloadNav(u,node){if(!downloadPage()||!u)return false;var a=abs(u),h=host(a),c=host(location.href);if(!cleanDownload(a))return false;if(asset(a))return true;if(h&&c&&same(h,c))return true;return downloadControl(node)&&trustedDownloadHost(h);}"
