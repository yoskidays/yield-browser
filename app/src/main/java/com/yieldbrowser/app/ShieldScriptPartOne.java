...
final class ShieldScriptPartOne {
    ...
    static String value() {
        return "function low(v){return String(v||'').toLowerCase();}"
                ...
                + "function downloadControl(node){try{var el=node&&node.closest?node.closest('a[href],area[href],button,[role=button],form[action]'):null;if(!el)return false;var m=navMeta(el);if(/(?:^|[ _-])(ad|ads|advert|sponsor|promo|affiliate|popup|popunder)(?:[ _-]|$)/i.test(m))return false;var named=/(?:^|[ _-])(download|unduh|save|mirror|server|quality|resolution|batch|gofile|mediafire|google[ _-]*drive|drive|mega|pixeldrain|dropbox|onedrive|480p|720p|1080p|2160p)(?:[ _-]|$)/i.test(m);return !!(el.hasAttribute&&el.hasAttribute('download'))||(named&&contentRegion(el));}catch(e){return false;}}"
                + "function cleanDownload(u){var a=abs(u),h=host(a);return /^https?:/i.test(a)&&!!h&&!adHost(h)&&!cheap(h)&&!hardToken(a)&&!redirParam(a)&&!relay(a);}"
                + "function safeDownloadNav(u,node){if(!downloadPage()||!u)return false;var a=abs(u),h=host(a),c=host(location.href);if(!cleanDownload(a))return false;if(asset(a))return true;if(h&&c&&same(h,c))return true;return trustedDownloadHost(h)&&(downloadControl(node)||Date.now()<=(S.downloadClickUntil||0));}"
